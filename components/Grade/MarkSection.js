...
export default function MarkSection(props) {
  ...
  return (
    <AnimatePresence>
      <motion.div layout className={`${styles.markParent} m-1 ${tooltipMovementStyles}`} >
        {/* <motion.div layout className={`${styles.markTriangle}`} /> */}
        <motion.div layout className={`${styles.markBody} ${tooltipColorStyles}`}>
          ...
          {isCollapsed ? null :
            <motion.div className='flex justify-end mt-3' key={isCollapsed} transition={collapseTransition} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
              ...
              <div className='flex items-end'>
                ...
                <svg onClick={goToClaude} className={`${styles.claude} w-6 m-1`} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100" fill="currentColor">
                ...
                <svg onClick={goToGPT} className={`${styles.chatgpt} w-6 m-1`} viewBox='0 0 320 320' xmlns='http://www.w3.org/2000/svg' fill='currentColor' >
