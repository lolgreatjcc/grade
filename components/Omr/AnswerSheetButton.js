...
export default function AnswerSheetButton({handleFile, setAnswerSheet, previewImage, setPreviewImage}) {
  const [fileStatus, setFileStatus] = useState(null);

  // Runs handle file found in the landing index.js
  const handleAnswerSheet = async (event) => {
    setFileStatus( await handleFile(event, setAnswerSheet, setPreviewImage));
  }
  ...
  return (
    <div className={`${showFileStatus(fileStatus)} bg-white w-70 h-99 rounded-md m-8 relative overflow-hidden`}>
      ...
      <div className={`${styles.overlay}`}>
        <input className={`${styles.fileInput}`} type="file" accept="*" onChange={handleAnswerSheet} color="rgba(0,0,0,0)"/>
