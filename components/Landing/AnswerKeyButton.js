...
export default function AnswerKeyButton({handleFile, setAnswerKey}) {

  const [fileStatus, setFileStatus] = useState(null);

  const handleAnswerKey = async (event) => {
    setFileStatus(handleFile(event, setAnswerKey));
  }
  ...
  return (
    <div className={`${showFileStatus(fileStatus)} bg-white w-70 h-99 rounded-md m-8 relative overflow-hidden`}>
      ...
      <div className={`${styles.overlay}`}>
        <input className={`${styles.fileInput}`} type="file" accept=".pdf" onChange={handleAnswerKey} color="rgba(0,0,0,0)"/>
