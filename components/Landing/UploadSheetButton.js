...
export default function UploadSheetButton({handleFile, setSheet, caption}) {
  ...
  return (
    <div className={`${showFileStatus(fileStatus)} bg-white w-70 h-99 rounded-md mx-8 my-4 relative overflow-hidden`}>
      <Image className="max-w-100 max-h-100" alt="img" fill={true} src={previewImage ? previewImage : 'https://picsum.photos/200/300'}/>
      <div className={`${styles.overlay}`}>
        <input className={`${styles.fileInput}`} type="file" accept=".pdf" onChange={handleSheet} color="rgba(0,0,0,0)"/>
