...
const htmlToLocal = async (html, fileName) => {
    const browser = await puppeteer.launch();
    const page = await browser.newPage();
    await page.setContent(html, { waitUntil: 'networkidle0' })
    page.pdf({'path': './mediaUploadTemp/' + fileName, 'format': 'A4', 'printBackground': true})
}

const generateHtml = ({numberOfMcqs, numberOfOptions, institution, subject, year, duration, answers}) => {
    //------------------------ Testing answer sheet generation on backend -----------------------//
    let tempContent = "";
...
const generateAnswerSheetMcq = (options, filename) => {
    const html = generateHtml(options)
    htmlToLocal(html, filename);
}

module.exports = generateAnswerSheetMcq;